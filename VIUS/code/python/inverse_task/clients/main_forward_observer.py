...
from forward_winding.forward_rhs_calculator import DelegatingForwardRHS, ForwardRHS
from forward_winding.forward_winding_builder import ForwardWindingBuilder
from observers.diagnostics_collector import DiagnosticsCollector
from solvers.linear_solver import GMRESSolver
...
tan_theta = 0.02       # для геодезической (можно 0.1 для винтового эффекта)
...
v_start = balloon.v_min +0.05  # отступим от самого низа, чтобы избежать полюса
u0 = 0
v0 = v_start
...
lin_solver.add_observer(collector)
ode_solver = SciPySolver(method='DOP853')
ode_solver.add_observer(collector)

rhs = DelegatingForwardRHS(surface, deviation_law, lin_solver, eps=1e-12)
...
#     eps=1e-12
# )
builder = ForwardWindingBuilder(surface, deviation_law, ode_solver, rhs=rhs)
...
if builder.last_run_successful:
    ...
    # exit(1)
if not builder.last_run_successful:
    diag = builder.get_diagnostics()
    print("Причина:", diag['message'])
    print("Исключение:", diag.get('exception'))
    exit(1)
# ----------------------------------------------------------------------
