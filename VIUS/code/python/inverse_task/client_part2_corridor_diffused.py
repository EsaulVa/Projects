...
# -*- coding: utf-8 -*-
"""
client_part2_corridor_diffused.py
=================================
Обратная задача DAE для corridor_3 с диффузионной фильтрацией оправки E2.

Параметры фильтра (tau, n_steps) вынесены в начало main() — подбирайте
по графикам из client_visualize_diffusion.py.
"""
...
class DAEPredictorSafe(DAEPredictor):
    def __init__(self, max_speed=10.0, max_mu=100.0, ng_threshold=1e-10):
    ...
    def compute_mu(self, R_prime, V_thread):
        ...
        if Ng < self.ng_threshold:
            return 0.0
        
        mu = -(dphi + float(grad_u @ Rp)) / Ng
        # Жёсткое ограничение mu
        if abs(mu) > self.max_mu:
            mu = self.max_mu * (1.0 if mu > 0 else -1.0)
        return mu
# =============================================================================
# Адаптер для внешней поверхности (E1) — остается численным
# =============================================================================
...
# =============================================================================
# Проекция точки на сглаженную поверхность вращения
# =============================================================================
def project_point_to_diffused(surface: DiffusedRevolutionSurface, point_3d):
    """
    Для поверхности вращения: v = atan2(y,x), s — длина дуги меридиана.
    Ищем s, минимизируя расстояние до меридиана v=0 (и v=π, выбираем ближайший).
    """
    x, y, z = point_3d
    v0 = np.arctan2(y, x)
    # Проверим два меридиана: v0 и v0+π (симметрия)
    candidates = []
    for v in [v0, v0 + np.pi]:
        def dist_s(s):
            try:
                r = surface.position(s, v)
            except Exception:
                return 1e9
            return np.linalg.norm(r - point_3d)
        s_min, s_max = surface.u_min, surface.u_max
        res = minimize_scalar(dist_s, bounds=(s_min, s_max), method='bounded')
        candidates.append((res.x, v, res.fun))
    best = min(candidates, key=lambda t: t[2])
    s_opt, v_opt, err = best
    r_opt = surface.position(s_opt, v_opt)
    return s_opt, v_opt, r_opt, err
...
# =============================================================================
def main():
    # --- ПАРАМЕТРЫ ФИЛЬТРА (подбирайте по графикам) ---
    FILTER_N = 800          # точек меридиана
    FILTER_TAU = 2.0        # шаг диффузии, мм²
    FILTER_NSTEPS = 5       # число шагов → полное время t = 10 мм²
    ...
    # --- ФИЛЬТРАЦИЯ ОПРАВКИ ---
    print("=" * 70)
    print("Диффузионная фильтрация оправки E2")
    print(f"  tau={FILTER_TAU}, n_steps={FILTER_NSTEPS}, N={FILTER_N}")
    print(f"  Полное время t = {FILTER_TAU * FILTER_NSTEPS:.1f} мм²")
    print("=" * 70)
    E2 = DiffusedRevolutionSurface(E2_raw, N=FILTER_N, tau=FILTER_TAU, n_steps=FILTER_NSTEPS)
    print(f"E2 отфильтрована: s ∈ [{E2.u_min:.1f}, {E2.u_max:.1f}] мм")
    ...
    print(f"Интерполировано на {N_fine} точек")
    ...
    # --- Начальное условие: проекция R0 на отфильтрованную E2 ---
    R0 = tsn_fine[2000]
    u0, v0, r0_proj, err_proj = project_point_to_diffused(E2, R0)
    print(f"Проекция R0 на E2: s0={u0:.4f}, v0={v0:.4f}, ошибка={err_proj:.4f} мм")
    ...
    # predictor = DAEPredictorSafe(max_speed=10.0, max_mu=50.0, ng_threshold=1e-10)
    ...
    print(f"Восстановленная ЛУ: {N} точек, длина s={s_hist[-1]:.2f} мм")
    ...
    df_out.to_csv("part2_corridor_diffused.csv", index=False)
    print(f"CSV: part2_corridor_diffused.csv")
    ...
    fig.update_layout(
        title=f"Обратная задача DAE (corridor_3, diffused E2, t={FILTER_TAU*FILTER_NSTEPS:.1f} мм²)",
        scene=dict(xaxis_title="X", yaxis_title="Y", zaxis_title="Z", aspectmode="data"),
        width=1200, height=900
    )
    fig.write_html("part2_corridor_diffused_3d.html")
    print("3D: part2_corridor_diffused_3d.html")
    fig.show()
    ...
    plt.tight_layout()
    plt.savefig("part2_corridor_diffused_diagnostics.png", dpi=150)
    print("Графики: part2_corridor_diffused_diagnostics.png")
    plt.show()
