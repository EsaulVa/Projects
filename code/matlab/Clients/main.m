...
clean_y = r * sin(theta);
% clean_x=[-1; clean_x]'
% clean_y=[r-0.01; clean_y]'
% Çàøóìëåííûå äàííûå
noise_std = 1e-3;
points_noisy = [clean_x + noise_std*randn(length(clean_x), 1), ...
                clean_y + noise_std*randn(length(clean_x), 1)];

%% Ãğàíè÷íûå óñëîâèÿ
bc_start = struct('direction', [0.0,1], 'curvature', 1.0/r);
bc_end = struct('direction', [-1,0], 'curvature', 1.0/r);
...
for i = 1:length(alphas)
    fprintf('\n=== alpha = %.3f ===\n', alphas(i));
    spline_temp = Splines.ParametricQuinticSpline(points_noisy, bc_start, bc_end);
...
yp_combined=tangents(:, 2);
xp_combined=tangents(:, 1);
ypp_combined=second_derivs(:, 2);
xpp_combined=second_derivs(:, 1);
