classdef BoundaryCondition < handle
    ...
    methods (Static)
        function bc = dirichlet(val)
            %DIRICHLET h = val
            bc = BoundaryCondition('dirichlet', val, 1.0, 0.0);
        end

        function bc = neumann(flux)
            %NEUMANN dh/ds = flux
            bc = BoundaryCondition('neumann', flux, 1.0, 0.0);
        end

        function bc = robin(a, b, val)
            %ROBIN a*h + b*dh/dn = val
            bc = BoundaryCondition('robin', val, a, b);
        end
    end
    ...
    methods
        ...
        function bc = with_value(obj, new_val)
            ...
            bc = BoundaryCondition(obj.kind, new_val, obj.coeff_a, obj.coeff_b);
        end
    end
end
