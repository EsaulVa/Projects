classdef DiffusedRevolutionSurface < handle
    ...
    %      E2_smooth = DiffusedRevolutionSurface(E2_raw, 800, 2.0, 5, ...
    %          BoundaryCondition.dirichlet(0), BoundaryCondition.dirichlet(0), ...
    %          'PreserveZParameter', true, 'SaveMeridianPath', 'out.csv');
    %      pt = E2_smooth.position(100.0, 0.0);
    %      r_z = E2_smooth.radius(400.0);   % только при PreserveZParameter=true
    ...
    methods
        function obj = DiffusedRevolutionSurface(base_surface, N, tau, n_steps, ...
            ...
            % --- 6. Опционально: сохранить z-параметризацию ---
            if obj.preserve_z
        ...
        function d = derivatives_by_z(obj, z, v)
            s = obj.s_from_z(z);
            r = ppval(obj.pp_r, s);
            dr_ds = ppval(obj.pp_dr, s);
            dz_ds = ppval(obj.pp_dz, s);
            if abs(dz_ds) < 1e-14
                dr_dz = 0.0;
            else
                dr_dz = dr_ds / dz_ds;
            end
            ru = [dr_dz * cos(v), dr_dz * sin(v), 1.0];
